...
from src.differential_linear_distinguisher import (
    run_refined_search_and_verify,
    run_search_and_verify,
    search_practical_distinguishers,
)
...
class DistinguisherPipelineTests(unittest.TestCase):
    ...
    def test_refined_result_files_generated_by_small_smoke_run(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            refined_results, verified = run_refined_search_and_verify(
                refined_search_samples=8,
                verify_samples=16,
                seed=888,
                output_dir=output_dir,
                max_input_masks=3,
            ...
            expected_files = [
                output_dir / "salsa20_refined_search_results.csv",
                output_dir / "salsa20_refined_verified_distinguisher.csv",
                output_dir / "salsa20_refined_verified_distinguisher.json",
            ]
            for path in expected_files:
                self.assertTrue(path.exists(), f"missing output file: {path}")

            payload = json.loads(
                (output_dir / "salsa20_refined_verified_distinguisher.json").read_text()
            )
            self.assertEqual(payload["results"][0], verified)
            self.assertLessEqual(len(refined_results), 4)
            self.assertIn("correlation", verified)
